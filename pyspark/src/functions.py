...
df.withColumn('Gender', regexp_replace(col('Gender'), 'Female', 'F')) \
  .withColumn('Gender', regexp_replace(col('Gender'), 'Male', 'M'))
  
# Convertendo os valores de uma coluna para maiúsculo e minúsculo
# Lower (minúsculo)
df.select(col('Gender'),lower(col('Gender'))).withColumnRenamed('lower(Gender)','Lower')

# Upper (maiúsculo)
df.select(col('Gender'),upper(col('Gender'))).withColumnRenamed('upper(Gender)','Upper')
