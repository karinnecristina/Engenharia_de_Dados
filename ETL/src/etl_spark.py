...
def validation(df:DataFrame,date_column:list[str],id_column:list[str]) -> DataFrame:
    ...
    for item in date_column:
        ...
        if dict(df.dtypes)[item] == 'string':
            raise TypeError(f'Erro: O tipo de dado da coluna "{item}" deve ser datetime64[ns].')
        else:
...
def clean(df:DataFrame,replaceCols:list[str]) -> DataFrame:
    ...
    for cols in replaceCols:
       df=df.withColumn(cols,when((col(cols) == "#REF!") | (col(cols) == '***'),None).otherwise(col(cols)))
